...
func CheckTeamDrive(teamDrive *drive2.Drive, driveApi *drive.Api, adminApi *admin.Api, wait *sync.WaitGroup) {
	...
	for role := range users {
		...
		checkGroupExists:
			...
			exists, err := adminApi.GroupExists(groupAddress + "@" + App.AppConfig.Domain)
			if err != nil {
				logrus.Error(err)
			...
			if !exists { //&& AskBool(fmt.Sprintf("Cant find group for Teamdrive `%s` with Role `%s`. Should it be created?", teamDrive.Name, role), true) {
				_, err := adminApi.CreateGroup(fmt.Sprintf("%s | %s", strings.ToUpper(string(role[0]))+role[1:], teamDrive.Name), groupAddress+"@"+App.AppConfig.Domain)
				if err != nil {
					logrus.Error(err)
					goto checkGroupExists
				}
	...
	listMembers:
		...
		groupMembers, err := adminApi.ListMembers(groupAddress + "@" + App.AppConfig.Domain)
		if err != nil {
			logrus.Error(err)
			goto listMembers
		}
		...
		for _, member := range members {
			...
			removeUser:
				...
				err := adminApi.RemoveMember(groupAddress+"@"+App.AppConfig.Domain, member)
				if err != nil {
					logrus.Error(err)
					goto removeUser
				}
	...
	listFunc := driveApi.ListPermissions
	if useDomainWide {
		listFunc = driveApi.ListPermissionsAdmin
	}

	deleteFunc := driveApi.DeletePermission
	if useDomainWide {
		deleteFunc = driveApi.DeletePermissionAdmin
	}

	createFunc := driveApi.CreatePermission
	if useDomainWide {
		createFunc = driveApi.CreatePermissionAdmin
	}

listPermissions:
	logrus.Infof("Listing Permissions for TeamDrive `%s`", teamDrive.Name)
	permissions, err := listFunc(teamDrive.Id)
	if err != nil {
		logrus.Error(err)
	...
	createPermission:
		...
		_, err := createFunc(teamDrive.Id, role, address, "group")
		if err != nil {
			logrus.Error(err)
	...
	deletePermission:
		...
		err := deleteFunc(teamDrive.Id, permissionId)
		if err != nil {
			logrus.Error(err)
